...
namespace SuperScrollView
{
    ...
    {
        public GameObject mItemPrefab = null;       //单项预制体
        public float mPadding = 0;
        public int mInitCreateCount = 0;           //初始化创建数
        public float mStartPosOffset = 0;          //其实位置偏移
    }
    ...
    public enum SnapStatus//对齐状态
    {
        NoTargetSet = 0,    //没有状态
        TargetHasSet = 1,   //设定目标
        SnapMoving = 2,     //快速移动
        SnapMoveFinish = 3  //快速移动完成
    }
    ...
    public class LoopListView2 : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        Dictionary<string, ItemPool> mItemPoolDict = new Dictionary<string, ItemPool>();//对象池字典
        List<ItemPool> mItemPoolList = new List<ItemPool>();//对象池列表
        [SerializeField]
        List<ItemPrefabConfData> mItemPrefabDataList = new List<ItemPrefabConfData>();//预设体信息表
       

        [SerializeField]
        private ListItemArrangeType mArrangeType = ListItemArrangeType.TopToBottom;     //表状态从上到下
        public ListItemArrangeType ArrangeType { get { return mArrangeType; } set { mArrangeType = value; } }//设置表排列状态
  
        List<LoopListViewItem2> mItemList = new List<LoopListViewItem2>();//需要显示的Grid列表
        RectTransform mContainerTrans;  //可以滚动的内容   Content
        ScrollRect mScrollRect = null;  //滚动列表
        RectTransform mScrollRectTransform = null;      //滚动条transform
        RectTransform mViewPortRectTransform = null;    //滚动条vireport rectTransform
        float mItemDefaultWithPaddingSize = 20;         //默认填充大小
        int mItemTotalCount = 0;                        //item总数
        bool mIsVertList = false;                       //是否是垂直列表
        System.Func<LoopListView2, int, LoopListViewItem2> mOnGetItemByIndex;//委托 根据索引得到Item
        Vector3[] mItemWorldCorners = new Vector3[4];  //Item世界角落
        Vector3[] mViewPortRectLocalCorners = new Vector3[4];//视图端口矩形角落
        int mCurReadyMinItemIndex = 0;      //当前准备的最小Item的索引 
        int mCurReadyMaxItemIndex = 0;      //当前准备的最大Item的索引 
        bool mNeedCheckNextMinItem = true;  //是否需要检查下一个最小Item
        bool mNeedCheckNextMaxItem = true;  //是否需要检查下一个最大Item
        ItemPosMgr mItemPosMgr = null;      //布局大小管理
        float mDistanceForRecycle0 = 300;   //回收距离0
        float mDistanceForNew0 = 200;       //创建距离0
        float mDistanceForRecycle1 = 300;   //回收距离1
        float mDistanceForNew1 = 200;       //创建距离1
        [SerializeField]
        bool mSupportScrollBar = true;      //是否使用全局滚动条
        bool mIsDraging = false;            //是否拖动
        PointerEventData mPointerEventData = null;  //鼠标事件
        public System.Action mOnBeginDragAction = null; //开始拖动委托
        public System.Action mOnDragingAction = null;   //拖动中委托
        public System.Action mOnEndDragAction = null;   //拖动结束委托
        int mLastItemIndex = 0; //最后Item索引
        float mLastItemPadding = 0;//最后Item填充
        float mSmoothDumpVel = 0;
        float mSmoothDumpRate = 0.3f;
        float mSnapFinishThreshold = 0.1f;
        float mSnapVecThreshold = 145;
        [SerializeField]
        bool mItemSnapEnable = false;           //是否使用Item对齐


        Vector3 mLastFrameContainerPos = Vector3.zero;//最后一帧容器的位置
        public System.Action<LoopListView2,LoopListViewItem2> mOnSnapItemFinished = null;//Item对齐完成委托
        public System.Action<LoopListView2, LoopListViewItem2> mOnSnapNearestChanged = null;//当最近的对齐改变委托
        int mCurSnapNearestItemIndex = -1; //当前对齐最近的项目索引
        Vector2 mAdjustedVec;//调整后的矢量
        bool mNeedAdjustVec = false;//是否需要调整矢量
        int mLeftSnapUpdateExtraCount = 1;//更新额外数量的左对齐
        [SerializeField]
        Vector2 mViewPortSnapPivot = Vector2.zero;//
        [SerializeField]
        Vector2 mItemSnapPivot = Vector2.zero;
        ClickEventListener mScrollBarClickEventListener = null;//滚动条监听
        SnapData mCurSnapData = new SnapData(); //临时数据
        Vector3 mLastSnapCheckPos = Vector3.zero;
        bool mListViewInited = false; //列表视图是否初始化
        int mListUpdateCheckFrameCount = 0;//列表更新检查帧计数
        ...
        public ItemPrefabConfData GetFirstItemPrefabConfData()
        {
            for(int i = 0; i < mItemPrefabDataList.Count; i++)
            {
                if(mItemPrefabDataList[i].mItemPrefab != null)
                {
                    return mItemPrefabDataList[i];
                }
            }
        ...
        public void OnItemPrefabChanged(string prefabName)
        {
            ...
            if(mItemList.Count > 0)
            {
                ...
                pos = mItemList[0].CachedRectTransform.anchoredPosition3D;//RectTransform.anchoredPosition3D 锚点的3D空间位置
            }
            RecycleAllItem();
        ...
        itemTotalCount: the total item count in the listview. If this parameter is set -1, 
        then means there are infinite items, and scrollbar would not be supported, and the ItemIndex can be from –MaxInt to +MaxInt. 
        If this parameter is set a value >=0 , then the ItemIndex can only be from 0 to itemTotalCount -1.
        onGetItemByIndex: when a item is getting in the scrollrect viewport, 
        and this Action will be called with the item’ index as a parameter, 
        to let you create the item and update its content.
        */
        //初始化
        //itemTotalCount如果为-1则代表无限滚动，不支持滚动条，如果大于0则支持滚动条
        //onGetItemByIndex 接口用来更新创建的Item
            ...
            LoopListViewInitParam initParam = null)
        {
            ...
            mScrollRect = gameObject.GetComponent<ScrollRect>();//得到滚动条
            if (mScrollRect == null)
            {
            ...
            mCurSnapData.Clear();   //
            mItemPosMgr = new ItemPosMgr(mItemDefaultWithPaddingSize);//得到位置管理器
            mScrollRectTransform = mScrollRect.GetComponent<RectTransform>();//得到滚动条的RectTransform
            mContainerTrans = mScrollRect.content;//得到滚动内容
            mViewPortRectTransform = mScrollRect.viewport;//得到滚动条viewport
            ...
            mIsVertList = (mArrangeType == ListItemArrangeType.TopToBottom || mArrangeType == ListItemArrangeType.BottomToTop);//得到是否是垂直滚动
            mScrollRect.horizontal = !mIsVertList;//设置是否启用水平滚动条
            mScrollRect.vertical = mIsVertList;//设置是否启用垂直滚动条
            SetScrollbarListener();
            AdjustPivot(mViewPortRectTransform);//设置滚动中心点
            AdjustAnchor(mContainerTrans);//设置滚动条内容锚点
            AdjustContainerPivot(mContainerTrans);//设置滚动条内容中心
            InitItemPool();
            mOnGetItemByIndex = onGetItemByIndex;//实现委托
            ...
            mCurSnapData.Clear();
            mItemTotalCount = itemTotalCount;//数值Item的总数量
        ...
        */
        /*此方法可用于在运行时设置scrollview的项总数。
            如果这个参数设置为-1，则表示有无限项，
            不支持滚动条，ItemIndex可以从-MaxInt到+MaxInt。
            如果将该参数设置为值>=0，那么ItemIndex只能从0到itemTotalCount -1。
            如果resetPos设置为false，则scrollrect的内容位置在此方法完成后不会更改。*/
        public void SetListItemCount(int itemCount, bool resetPos = true)
        {
            ...
            if (mSupportScrollBar)
            {
                mItemPosMgr.SetItemMaxCount(mItemTotalCount);//设置最大长度
            }
            else
            {
                mItemPosMgr.SetItemMaxCount(0);
            ...
            if (mItemTotalCount == 0)
            {
                ...
                RecycleAllItem();//删除所有Item
                ClearAllTmpRecycledItem();//清空Tmp对象池表
                UpdateContentSize();
                return;
            }
        ...
        //得到Viewport大小  如果是垂直滚动条返回高度，如果是水平滚动条返回宽度
        ...
        所有可见项存储在一个名为mItemList的列表中;
        此方法是通过可见项列表中的索引获取可见项。参数索引从0到mItemList.Count。
             */
        ...
        /// <summary>
        /// 当前scrollRect的voewPort区域
        /// </summary>
        Vector3[] viewportWorldCorners = null;

        /// <summary>
        /// 获取在viewport中的第一个对象
        /// </summary>
        /// <returns></returns>
        public LoopListViewItem2 GetViewportFirstItem()
        {
            if (mItemList.Count == 0)
            {
                return null;
            ...
            mItemWorldCorners = mItemWorldCorners ?? new Vector3[4];
            viewportWorldCorners = viewportWorldCorners ?? new Vector3[4];
            ...
            for(int i = 0; i < mItemList.Count; i++)
            {
                ...
                item.GetComponent<RectTransform>().GetWorldCorners(mItemWorldCorners);
                if (ScrollRect.vertical)
                {
                    if (mItemWorldCorners[0].y <= viewportWorldCorners[i].y)
                    {
                        return item;
                    }
                }
                ...
                {
                    if (mItemWorldCorners[2].x >= viewportWorldCorners[0].x)
                    {
                        return item;
                    }
                }
        ...
        对于垂直滚动矩形，当一个可见项的高度在运行时发生变化时，应该调用这个方法，让LoopListView2组件重新定位所有可见项的位置。
        对于水平滚动矩形，当一个可见项的宽度在运行时发生变化时，应该调用这个方法来让LoopListView2组件重新定位所有可见项的位置。*/
        ...
        */
        ...
        该方法将scrollrect内容的位置移动到(itemIndex-th item的位置+偏移量)，偏移量从0到scrollrect视口大小。
        */
        public void MovePanelToItemIndex(int itemIndex, float offset)
        {
            mScrollRect.StopMovement();//滚动条停止运动
            mCurSnapData.Clear();//清空临时数据
            if (mItemTotalCount == 0)
            {
                return;
            }
            ...
            {//滚动到的索引大于最大数量，重赋值索引为最后一个Item的索引
            ...
            {
                //从上到下
                float containerPos = mContainerTrans.anchoredPosition3D.y;//content的y轴
                if (containerPos < 0)
                {
                    containerPos = 0;
                }
            ...
            newItem.CachedRectTransform.anchoredPosition3D = pos;//设置Item的位置
            if (mSupportScrollBar)
            {
                if (mIsVertList)
                {
        ...
        void InitItemPool()
        {
            foreach (ItemPrefabConfData data in mItemPrefabDataList)
            {
                ...
                if (mItemPoolDict.ContainsKey(prefabName))//判断name是否已经存在
                {
                ...
                pool.Init(data.mItemPrefab, data.mPadding,data.mStartPosOffset, data.mInitCreateCount, mContainerTrans);//把预设体添加到对象池
                mItemPoolDict.Add(prefabName, pool);//添加到字典
                mItemPoolList.Add(pool);//添加到列表
            }
        }
        ...
        //更新快速移动。如果立即设置为true，那么snap移动将立即完成。
        ...
        void UpdateSnapVertical(bool immediate = false, bool forceSendEvent = false)
        {
            ...
            int count = mItemList.Count;//得到总数
            if (count == 0)
            {
                return;
            }
            Vector3 pos = mContainerTrans.anchoredPosition3D;//content的坐标
            bool needCheck = (pos.y != mLastSnapCheckPos.y);//content坐标是否等于左后点击的坐标
        ...
        bool UpdateForVertList(float distanceForRecycle0,float distanceForRecycle1,float distanceForNew0, float distanceForNew1)
        {
            ...
            if (mArrangeType == ListItemArrangeType.TopToBottom)
            {
                ...
                if (itemListCount == 0)
                {
                    ...
                    if (mSupportScrollBar)
                    {
                        SetItemSize(index, newItem.CachedRectTransform.rect.height, newItem.Padding);//根据索引设置大小
                    }
                    mItemList.Add(newItem);
        ...
        float GetContentPanelSize()
        {
            ...
            int count = mItemList.Count;//item总数
            if (count == 0)
            {
                return 0;
            }
            if (count == 1)
            {
                return mItemList[0].ItemSize;//得到单个Item的大小   如果是垂直滚动 则返回Hight  水平滚动则返回Weight
            }
            if (count == 2)
            {
                return mItemList[0].ItemSizeWithPadding + mItemList[1].ItemSize;
            }
            float s = 0;//便是总长度或者总宽度
        ...
        void CheckIfNeedUpdataItemPos()
        {
            ...
            if (mArrangeType == ListItemArrangeType.TopToBottom)
            {
                ...
                float viewMaxY = GetContentPanelSize();//得到总长度（宽度）
                if (firstItem.TopY/*第一个Item最高点Y的坐标*/ > 0 || (firstItem.ItemIndex == mCurReadyMinItemIndex && firstItem.TopY != 0))
                {
                    UpdateAllShownItemsPos();
                    return;
                }
        ...
        {
            float size = GetContentPanelSize();//总长度或者总宽度
            if (mIsVertList)
            {
                if(mContainerTrans.rect.height != size)
                {//如果高度不和总长度相等  赋值
                    mContainerTrans.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, size);//使RectTransform计算的rect在指定的轴上具有给定的大小。  参数（轴指定方向，沿指定轴的大小）
                }
            }
            else
            {
